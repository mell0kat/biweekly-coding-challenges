...
const interpretInstruction = (instructionValue) => {

  const opcode = +instructionValue.toString().slice(-2)
  const parameterCodes = reverse(instructionValue.toString().slice(0, -2).split(''))
  return {
    opcode,
    parameterCodes
  }
...
console.log(interpretInstruction(1101)) // { opcode: 1, parameterCodes: [ '1', '1' ] }
console.log(interpretInstruction(1002)) // { opcode: 2, parameterCodes: [ '0', '1' ] }
...
const restoreIntcode = (inputArray, inputValue) => {
  ...
  while (true) {
    ...
    const {
      opcode,
      parameterCodes
    } = interpretInstruction(instructions[i])
    ...
    const value1 = applyParameterMode(instructions, valueAt1, +parameterCodes[0])
    const value2 = applyParameterMode(instructions, valueAt2, +parameterCodes[1])
